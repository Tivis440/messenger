...
Современный кроссплатформенный мессенджер на Qt/C++ с обычными именами пользователей, обязательным TLS-транспортом, локальной книгой контактов, офлайн-доставкой, статусами сообщений и экспериментальной end-to-end моделью на базе `libsignal-protocol-c`.
...
- Отправка сообщений конкретному выбранному контакту по username, а не broadcast всем пользователям.
...
Имя используется для входа и добавления контактов. Допустимы буквы, цифры и символы:
...
После регистрации пользователь получает username. Его можно передать другому пользователю, чтобы тот добавил контакт вручную.
...
Это компактный, нативный и достаточно серьезно устроенный desktop messenger: Qt-клиент, TLS relay-сервер, username-аккаунты, локальные контакты, offline delivery, статусы сообщений и фокус на шифровании содержимого переписки.
