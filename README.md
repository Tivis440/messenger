...
Современный кроссплатформенный мессенджер на Qt/C++ с обязательным TLS-транспортом, локальной книгой контактов, офлайн-доставкой, статусами сообщений и экспериментальной end-to-end моделью на базе `libsignal-protocol-c`.
...
- Случайный публичный ID пользователя формата `nlk_...`.
- Локальные `@ники` как удобные алиасы без передачи display name на сервер.
- Ручное добавление и удаление контактов.
- Отправка сообщений конкретному выбранному контакту, а не broadcast всем пользователям.
...
- Минимизация метаданных: сервер не рассылает полный список пользователей и не служит публичным каталогом.
...
Клиент отвечает за пользовательский интерфейс, локальные контакты, локальные имена, историю диалогов и криптографическое состояние. Сервер выполняет роль защищенного relay: принимает TLS-подключения, аутентифицирует пользователей, хранит prekey bundles и доставляет offline-сообщения.

Сервер не должен знать локальные `@ники`, display names и содержимое сообщений. Аккаунт на сервере идентифицируется случайным публичным ID.
...
При регистрации клиент создает случайный публичный ID:

```text
nlk_6801CAE3B30D420DA1AB
```

Этот ID можно передать человеку вручную. Контакт добавляет ID в свою локальную книгу контактов и может назначить ему удобный локальный `@ник`, например:

```text
@kirill
```

`@ник` хранится только на устройстве пользователя. Сервер не индексирует ники, не знает display name и не является поисковой системой людей. Это снижает раскрытие социального графа.
...
- Сервер не отправляет полный список зарегистрированных пользователей.
- Контакты хранятся локально.
- Нет глобального broadcast online-статусов.
- Основной идентификатор аккаунта - random public ID, а не читаемое имя.
...
- Sealed Sender или близкая модель сокрытия отправителя.
- Приватный contact discovery вместо прямого запроса ID.
- Reproducible builds.
...
После регистрации пользователь получает публичный ID. Его можно передать другому пользователю, чтобы тот добавил контакт вручную.
...
Это компактный, нативный и достаточно серьезно устроенный desktop messenger: Qt-клиент, TLS relay-сервер, локальные контакты, offline delivery, статусы сообщений и постепенное движение к более приватной архитектуре без публичного каталога пользователей.
