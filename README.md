...
Клиент сам получает сертификат при первом подключении и сохраняет его SHA-256 fingerprint локально. При следующих подключениях смена сертификата будет считаться подозрительной и подключение остановится.

Если сервер был переустановлен или сертификат осознанно заменен, в клиенте можно открыть `Справка -> Сбросить сертификат сервера` и подключиться заново.
